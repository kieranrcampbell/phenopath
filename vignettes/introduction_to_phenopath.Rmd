...
devtools::load_all("~/oxford/phenopath/") # library(phenopath)
```
...
PhenoPath fits models with a call to the `phenopath` function, which requires at least an expression matrix `y` and a covariate vector `x`. For use with `ExpressionSets` see the [section on input formats](#inputdata). For this example we choose an ELBO tolerance of `1e-6` and ELBO calculations thinned by `40`. A discussion of how to control variational inference can be found [below](#varcontrol).
...
The posterior mean estimates of the pseudotimes $z$ sit in `fit$m_z`. We can visualise these compared to both the true pseudotimes and the first principal component of the data:
