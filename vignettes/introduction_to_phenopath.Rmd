...
For use with `SingleCellExperiment`s see the [section on input formats](#inputdata). For this example we choose an ELBO tolerance of `1e-6` and ELBO calculations thinned by `40`. A discussion of how to control variational inference can be found [below](#varcontrol).
...
## Using an `SingleCellExperiment` as input

Alternatively you might have expression values in an `SingleCellExperiment`. For single-cell data it is highly recommended to use the [SingleCellExperiment](https://bioconductor.org/packages/release/bioc/html/singlecellexperiment.html)  in which case data is stored in a class derived from `SummarizedExperiment`.

We'll first construct an example `SingleCellExperiment` using our previous simulation data:
...
Note that PhenoPath will use by default is in the `exprs` assay of a `SingleCellExperiment` (ie `assay(sce, "exprs")`) as gene expression values, which can be changed using the `sce_assay` string in the column to `phenopath`. 
